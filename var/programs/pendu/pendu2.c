...
int main (int argc, char *argv[]) {
    char configFileSrc[FILENAME_MAX] = "words.txt";
...
char loadConfig(char filePath[]) {
    FILE* configFile = NULL;
    configFile = fopen(filePath, "r");
    if(NULL == configFile) {
        return 0;
    }
    printf("File %c", filePath);
/*
    do {
        initPendu(pendu);
        do {    
    ...
    return getWordsFromFile(configFile);
    */
}

int processPendu(Pendu* pendu) {
